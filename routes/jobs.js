...
const jobModel = required("../models/job.model.js");
const applicationModel = required("../models/application.model.js");
const userModel = required("../models/user.model.js");
const authMiddleware = required("../middleware/auth.js");
...
router.post("/", authMiddleware, async (req, res, next) => {
    try{
        ...
        const serializedSkills = skills.split(",").map(skills.trim());
        ...
        const updatedUser = await userModel.findByIdUpdated(req.user.id, { $push: { createdJobs: job._id}});
        ...
        await job.save();
        // res.json({ message: "Jon created Successfully"}).status(200);
